require 'rails_helper'

describe PublicKey do
  context "when given an invalid public key value format" do
    it "returns a validity state of false" do
      pk = PublicKey.new(value: " adeponte@andrew-de-pontes-macbook-pro.local")
      expect(pk.valid?).to eq(false)
    end
  end

  context "when given a valid public key value format and public key is unique" do
    it "returns a validity state of true" do
      pk = PublicKey.new(value: "ssh-dss  adeponte@andrew-de-pontes-macbook-pro.local")
      expect(pk.valid?).to eq(true)
    end
  end

  context "when public key is not unique" do
    it "returns a validity state of false" do
      PublicKey.create(name: "test" , value: "ssh-dss  adeponte@andrew-de-pontes-macbook-pro.local")
      pk = PublicKey.new(value: "ssh-dss  adeponte@andrew-de-pontes-macbook-pro.local")
      expect(pk.valid?).to eq(false)
    end
  end
end
